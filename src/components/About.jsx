...
import {faGraduationCap, faPeopleGroup, faMedal} from "@fortawesome/free-solid-svg-icons";
...
function About() {
    ...
    return (<div className="h-screen flex justify-center items-center text-blue-950 py-2 mx-96 gap-10">
        <div className={"flex justify-center items-center w-1/2"}>
            <img src={aboutImage} alt="Hei students having fun" className={"w-[530px]"}/>
        </div>
        <div className={"flex flex-col gap-10 w-1/2"}>

            <h2 className="text-5xl font-serif font-bold">À propos de nous</h2>

            {textContent.map((item, index) => (
                <div className={"flex flex-col gap-5 px-2"} key={index}>
                    <div className={"flex items-center gap-2"}>
                        <FontAwesomeIcon icon={item.icon} size={"4x"} className={"aspect-square text-amber-400"}/>
